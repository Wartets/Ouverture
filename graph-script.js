window.addEventListener("backgrounds-loaded", () => {
	...
	node.append("circle")
		...
		.attr("stroke", d => (
			!d.id.startsWith("obj") && window.missingBackgroundScenes.has(d.id) ? "red" : "none"
		))
		.attr("stroke-width", d => (
			!d.id.startsWith("obj") && window.missingBackgroundScenes.has(d.id) ? 1 : 0
		));
	...
	const numScenesWithBackground = numScenes - window.missingBackgroundScenes.size;
	...
	function expandHud() {
		if (collapsed) {
			...
			hud.innerHTML = `
				...
				${window.missingBackgroundScenes.size > 0
					? `Scenes with background: ${numScenesWithBackground}/${numScenes}<br>`
					: ''
				}
			`;
		}
